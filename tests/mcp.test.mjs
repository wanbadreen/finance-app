      ...
      from(bucket) {
        ...
        return {
          async upload(path, body, options) {
            storageFiles.set(path,{body,options});
            return {data:{path},error:null};
          },
          async remove(paths) {
            for (const path of paths) storageFiles.delete(path);
...
test('receipt input schema requires ChatGPT file id and download URL but keeps name/type optional',()=>{
  assert.throws(()=>schemas.attach_receipt_to_transaction.parse({
    transaction_id:'00000000-0000-4000-8000-000000000040',
    receipt:{file_id:'file_test'},
    confirmed:true
  }));
  const parsed=schemas.attach_receipt_to_transaction.parse({
    transaction_id:'00000000-0000-4000-8000-000000000040',
    receipt:{file_id:'file_test',download_url:'https://files.example.test/receipt'},
    confirmed:true
  });
  assert.equal(parsed.receipt.file_id,'file_test');
});

test('attach receipt downloads a valid image, uploads to user receipt folder and links transaction',async()=>{
  const txId='00000000-0000-4000-8000-000000000041';
  const db=database({
    transactions:[{
      id:txId,user_id:user,account_id:accountId,category_id:categoryId,type:'expense',
      amount:10,transaction_date:'2026-09-24',description:'Dinner',notes:null,receipt_path:null,
      deleted_at:null,recurring_id:null,linked_transfer_id:null
    }]
  });
  const previousFetch=globalThis.fetch;
  globalThis.fetch=async()=>new Response(
    new Uint8Array([0x89,0x50,0x4e,0x47,0x0d,0x0a,0x1a,0x0a,0x00]),
    {status:200,headers:{'content-length':'9','content-type':'image/png'}}
  );
  try {
    const r=await reader(db,user)('attach_receipt_to_transaction',{
      transaction_id:txId,
      receipt:{
        download_url:'https://files.example.test/receipt.png',
        file_id:'file_receipt_1',
        mime_type:'image/png',
    ...
    assert.equal(r.replaced_existing_receipt,false);
    assert.equal(r.transaction.receipt_file_name,'receipt.png');
    assert.match(db.tables.transactions[0].receipt_path,new RegExp('^'+user+'/\\d{4}-\\d{2}-\\d{2}/.+\\.png
  const db=database({
...
test('create transaction can persist a confirmed ChatGPT receipt attachment',async()=>{
  const db=database({
    accounts:[{id:accountId,user_id:user,name:'Maybank',account_type:'bank',opening_balance:0,is_active:true}],
    categories:[{id:categoryId,user_id:user,name:'Food',type:'expense',is_active:true}],
    income_sources:[],payment_methods:[],transactions:[]
  });
  const previousFetch=globalThis.fetch;
  globalThis.fetch=async()=>new Response(
    new Uint8Array([0xff,0xd8,0xff,0x00]),
    {status:200,headers:{'content-length':'4','content-type':'image/jpeg'}}
  );
  try {
    const r=await reader(db,user)('create_transaction',{
      type:'expense',amount:18.5,transaction_date:'2026-09-25',description:'Dinner',
      account_id:accountId,category_id:categoryId,
      receipt:{
        download_url:'https://files.example.test/receipt.jpg',
        file_id:'file_receipt_2',
        file_name:'receipt.jpg'
      },
      confirmed:true
    });
    assert.equal(r.transaction.receipt_attached,true);
    assert.equal(r.transaction.receipt_mime_type,'image/jpeg');
    assert.ok(db.tables.transactions[0].receipt_path);
    assert.equal(db.storageFiles.size,1);
  } finally {
    globalThis.fetch=previousFetch;
  }
...
test('receipt upload rejects unsupported file bytes and does not mutate the transaction',async()=>{
  const txId='00000000-0000-4000-8000-000000000042';
  const db=database({
    transactions:[{
      id:txId,user_id:user,account_id:accountId,category_id:categoryId,type:'expense',
      amount:10,transaction_date:'2026-09-24',description:'Dinner',notes:null,receipt_path:null,
  ...
  try {
    await assert.rejects(reader(db,user)('attach_receipt_to_transaction',{
      transaction_id:txId,
      receipt:{download_url:'https://files.example.test/not-image',file_id:'file_bad'},
      confirmed:true
    }),/valid JPG, PNG, WebP or PDF/);
    assert.equal(db.tables.transactions[0].receipt_path,null);
